...
开始前：配置一个文本模型，准备至少包含目标消息的会话，并选中要修改的 `system` 或 `user` 消息。左侧得到该消息的优化稿；右侧运行时会把它放回完整会话。模型配置见[模型管理](../basic/models.md#first-text-model)。
...
**右侧只运行当前选中的消息吗？** 不是。右侧会执行整段会话，每列只替换当前选中的目标消息。

**左侧分析会读取右侧填写的变量值吗？** 不会。变量值用于右侧测试，左侧分析关注目标消息本身。
