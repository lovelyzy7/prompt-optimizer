...
开始前：按[模型管理](models.md#first-text-model)配置一个文本模型。左侧优化成功后会出现可复制的提示词；想看它生成的实际内容，再在右侧运行测试。第一次使用可直接跟随[快速开始](../user/quick-start.md)。
...
**为什么右侧没有额外的测试文本？** 因为在 `basic/user` 里，被测试的就是用户提示词本身。

**评估建议会直接修改历史版本吗？** 不会。应用建议时，系统会尝试修改左侧当前工作区。
